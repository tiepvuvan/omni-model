...
A config file has six top-level blocks plus a `version`. Every block has sane defaults, so the
smallest valid file is tiny — but always give `storage` an explicit `type`.
...
A minimal but complete skeleton that parses and starts:
...
| `allowUnauthenticated` | boolean | `false` | Serve `/v1/*` with no verifier. Startup fails without it. |

<Warning>
**At least one verifier is required.** With `providers: []` the proxy **refuses to start** — a proxy
that authenticates nobody is an open relay on your provider credits, and offers a caller nothing that
calling the upstream API directly wouldn't. Since that's the whole reason omni-model exists, it is
never the default.

For local development or a genuinely private network, opt in explicitly:
...
It logs a warning on every start, and `${...}` provider keys still apply — so an "open" proxy is
still spending real credits.
</Warning>

...
```yaml
- type: firebase-app-check
  # Omit projectNumber on Cloud Run / GCE when this is the Firebase project.
  projectNumber: "1234567890"
```

| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `projectNumber` | string | GCP metadata | The **numeric** Firebase project number (not the project id). Required outside Node deployments on GCP; set it explicitly for a different Firebase project. |
